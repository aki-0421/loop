...
Coding agents must use ` + "`loop task todo`" + ` for task-local work. Commit TODOs create task-branch commits, no_commit TODOs record clean validation or inspection work, and ` + "`loop task merge`" + ` merges the completed task branch with a CLI-generated ` + "`Task N done: <task title>`" + ` merge commit. If you exit before ` + "`loop task merge`" + ` succeeds, the orchestrator treats the attempt as unmerged, discards that task branch/worktree, and may retry the task in a fresh coding session.
