...
func allHelpCommands() []helpCommand {
	return []helpCommand{
		...
		},
		{
			...
			Description: "Builds each iteration prompt, launches the configured agent, validates the close handoff, and integrates merge closes.",
			Flags: []helpFlag{
		...
		},
		{
			...
			Description: "Agent-facing pull request lifecycle commands. In PR mode, run these before closing the iteration with --merge.",
			Agent:       true,
			AgentOnly:   true,
		},
		{
		...
		},
		{
			...
			Description: "Writes the terminal close handoff to the master DB. The command infers mechanical fields and returns actionable errors for contract problems before handoff.",
			Flags: append(iterationLocatorFlags(),
