...
func allHelpCommands() []helpCommand {
	return []helpCommand{
		...
		},
		{
			...
			Usage:       "loop task todo <add|list|move|remove|cancel|stage|start|complete> ...",
			Summary:     "Manage task-local TODO work",
			Description: "Task-local TODOs must be created before editing. Commit TODOs create CLI-owned commits; no_commit TODOs record work that must leave no repository changes.",
			Agent:       true,
			AgentOnly:   true,
		},
		{
			Path:        []string{"task", "todo", "add"},
			Usage:       "loop task todo add --work-type commit --type <type> --title <title> --acceptance <text>... [--after <n>] <commit-message>\nloop task todo add --work-type no_commit --title <title> --acceptance <text>... [--after <n>]",
			Summary:     "Add one task-local TODO",
			Description: "Adds a pending task TODO. Use --work-type no_commit for validation, inspection, or handoff work that should not create a commit. Use --after 0 to insert at the top before work starts. After work starts, new TODOs may be inserted only after the last done, active, or cancelled TODO.",
			Flags: append(iterationLocatorFlags(),
				helpFlag{Name: "--task <id>", Description: "task id"},
				helpFlag{Name: "--work-type <type>", Description: "work type: commit or no_commit", Default: "commit"},
				helpFlag{Name: "--type <type>", Description: "commit type: F, T, R, D, S, V, or C"},
		...
		},
		{
			...
			Description: "Reorders a pending task TODO. Pending TODOs may move only after the last done, active, or cancelled TODO.",
			Flags: append(iterationLocatorFlags(),
				helpFlag{Name: "--task <id>", Description: "task id"},
		...
		{
			Path:        []string{"task", "todo", "remove"},
			Usage:       "loop task todo remove <n> [--task <id>] [--iteration-dir <dir>|--run <run-id> --iteration <n>]",
			Summary:     "Remove a pending task-local TODO",
			Description: "Removes a pending task TODO from the queue. Done, active, and cancelled TODOs are fixed audit history and cannot be removed.",
			Flags: append(iterationLocatorFlags(),
				helpFlag{Name: "--task <id>", Description: "task id"},
			),
			Agent:     true,
		...
		{
			Path:        []string{"task", "todo", "cancel"},
			Usage:       "loop task todo cancel <n> [--discard-changes] [--task <id>] [--iteration-dir <dir>|--run <run-id> --iteration <n>]",
			Summary:     "Cancel a pending or active task-local TODO",
			Description: "Marks a pending or active TODO as cancelled. Active TODO cancellation requires --discard-changes, which discards task worktree and index changes before recording the cancellation.",
			Flags: append(iterationLocatorFlags(),
				helpFlag{Name: "--task <id>", Description: "task id"},
				helpFlag{Name: "--discard-changes", Description: "discard task worktree and index changes before cancelling an active TODO", Default: "false"},
			),
			Agent:     true,
			AgentOnly: true,
		},
		...
		{
			Path:        []string{"task", "todo", "stage"},
			Usage:       "loop task todo stage <n> [--add <path>]... [--remove <path>]... [--reset] [--task <id>] [--iteration-dir <dir>|--run <run-id> --iteration <n>]",
			Summary:     "Stage and inspect active task-local TODO files",
			Description: "Stages active TODO changes for review, prints staged commit candidates and remaining unstaged files, and lets agents add paths, remove paths from the index, or reset the index before completion.",
			Flags: append(iterationLocatorFlags(),
				helpFlag{Name: "--task <id>", Description: "task id"},
				helpFlag{Name: "--add <path>", Description: "stage this path; repeatable"},
				helpFlag{Name: "--remove <path>", Description: "unstage this path without deleting the worktree file; repeatable"},
				helpFlag{Name: "--reset", Description: "unstage all files before applying --add or --remove", Default: "false"},
			),
			Agent:     true,
			AgentOnly: true,
		},
		{
			...
			Description: "Commits the currently staged TODO changes, records the SHA, and marks the TODO done. Run `loop task todo stage <n>` first to inspect and adjust commit candidates.",
			Flags: append(iterationLocatorFlags(),
				helpFlag{Name: "--task <id>", Description: "task id"},
			),
			Agent:     true,
		...
		},
		{
			...
			Description: "Requires all task TODOs to be committed, serializes access to the iteration branch, merges the task branch with its commits preserved, and records task-merge.json. If conflicts are reported, resolve them in the printed iteration worktree and rerun with --continue.",
			Flags: append(iterationLocatorFlags(),
				helpFlag{Name: "--task <id>", Description: "task id"},
