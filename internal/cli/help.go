...
func allHelpCommands() []helpCommand {
	return []helpCommand{
		...
		{
			Path:        []string{"issue"},
			Usage:       "loop issue <ask> ...",
			Summary:     "Create GitHub clarification issues",
			Description: "Agent-facing clarification commands. Use only for important product, policy, or blocking specification questions.",
			Agent:       true,
			AgentOnly:   true,
		},
		{
