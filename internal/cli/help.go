...
func allHelpCommands() []helpCommand {
	return []helpCommand{
		...
		},
		{
			...
			Description: "Prints result JSON by default, or writes the result handoff to the master DB with --write. The command infers mechanical fields and returns actionable errors for contract problems.",
			Flags: append(iterationLocatorFlags(),
				helpFlag{Name: "--write", Description: "write generated JSON to the master DB result handoff", Default: "false"},
