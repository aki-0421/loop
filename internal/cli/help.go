...
func allHelpCommands() []helpCommand {
	return []helpCommand{
		...
		},
		{
			...
			Description: "Creates labels `loop:agent-gap` and `loop:proposal`, applies optional `loop:blocking`, embeds loop metadata, and prints the Issue reference. Use when the agent cannot inspect, validate, repair, or decide well because the repository is missing a tool, doc, guardrail, signal, environment affordance, or workflow.",
			Flags: append(iterationLocatorFlags(),
				helpFlag{Name: "--title <text>", Description: "Issue title"},
				helpFlag{Name: "--body <text>", Description: "Issue body with evidence and proposed improvement"},
				helpFlag{Name: "--body-file <path>", Description: "read the Issue body from a file"},
