...
func allHelpCommands() []helpCommand {
	return []helpCommand{
		...
		},
		{
			...
			Summary: "Inspect GitHub pull request memory",
			Agent:   true,
		},
		{
			Path:    []string{"memory", "recent"},
			Usage:   "loop memory recent [--repo <owner/name>] [--limit 30]",
			Summary: "Print recent GitHub pull request memory",
			Flags: []helpFlag{
				{Name: "--repo <owner/name>", Description: "limit to one GitHub repository"},
				{Name: "--limit <n>", Description: "maximum pull requests", Default: "config value"},
			},
			Agent: true,
		},
		{
			...
			Summary: "Search GitHub pull request titles and bodies",
			Flags: []helpFlag{
				{Name: "--repo <owner/name>", Description: "limit to one GitHub repository"},
