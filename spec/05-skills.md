...
---
name: loop
description: Execute one loop coding iteration using CLI-managed context, GitHub context memory, clarification Issues, commits, validation, PR text, and two-choice iteration close.
version: 1
---
```
...
The built-in `loop` skill consolidates the default iteration, GitHub context memory, clarification Issue, commit, pull request, and close conventions. It tells the agent how to:
...
- Create, check, fix, and merge pull requests with `loop pr` before closing PR-mode work with `--merge`.
...
- Size TODO items so one completed TODO maps to one `loop commit` invocation, except skip-merge confirmations.
...
- Rename mergeable-work branches with `loop branch rename` instead of direct Git branch commands.
- Write PR title and body artifacts from repository context and CLI-provided template text.
- Close the iteration through `loop iteration close --merge` or `loop iteration close --skip-merge` instead of hand-authoring close JSON.
