...
Writable artifacts are `plan`, `todo`, `worklog`, `pr-title`, and `pr-body`. Read-only artifacts include `runtime`, `instruction`, `prompt`, `effective-config`, `validation`, `events`, `stdout`, `stderr`, and `github-updates`. The result is written through the master-DB result handoff, not a local artifact.
