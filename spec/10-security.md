...
To keep behavior inspectable, every action must be recorded as an event. Push, pull request creation, check waiting, merge, cleanup, agent command execution, agent file reads, and filtered assistant-visible messages must appear in the iteration event log, a task event log, or a CLI event stream. Agent hidden thinking, raw transcripts, raw file contents, and raw diffs must not be persisted.
